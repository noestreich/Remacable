...
enum RmapiClient {
    ...
    private static func fetchLatestRelease() async throws -> [String: Any] {
        ...
        guard let http = response as? HTTPURLResponse, http.statusCode == 200,
            ...
            throw RmapiError.download(tr("error.rmapi.github_response"))
        }
        return json
    }
    ...
    static func latestVersion() async throws -> String {
        guard let tag = try await fetchLatestRelease()["tag_name"] as? String else {
            throw RmapiError.download(tr("error.rmapi.release_without_version"))
        }
        return tag
    }
    ...
    static func install(progress: @escaping (String) -> Void) async throws -> String {
        progress(tr("rmapi.install.finding_release"))
        ...
        guard let assets = json["assets"] as? [[String: Any]],
            ...
            throw RmapiError.download(tr("error.rmapi.no_asset", assetName))
        }
        let tag = (json["tag_name"] as? String) ?? "?"

        progress(tr("rmapi.install.downloading", assetName, tag))
        ...
        progress(tr("rmapi.install.extracting"))
        ...
        guard let extracted = try FileManager.default
            ...
            throw RmapiError.download(tr("error.rmapi.binary_missing"))
        }
        ...
        progress(tr("rmapi.install.installed", installed))
        return installed
    }
}
...
enum RmapiError: LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .notInstalled: return tr("error.rmapi.not_installed")
        case .notPaired: return tr("error.rmapi.not_paired")
        case .invalidCode: return tr("error.rmapi.invalid_code")
